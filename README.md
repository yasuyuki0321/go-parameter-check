...
```bash
go-parameter-test$ sam logs -n StockCheckerFunction --stack-name go-parameter-test --tail
```
...
```bash
go-parameter-test$ go test -v ./functions/...
```
...
```bash
aws cloudformation delete-stack --stack-name go-parameter-test
```
